...
## 概要
ESP32にLCDディスプレイとボタンを接続し、Wi-Fi経由でNAVITIME APIから乗り換え案内や遅延情報を取得して表示します。NTPサーバーで時刻を同期し、ボタン操作で路線や時刻表を切り替えることができます。また、バッテリー監視機能と、非アクティブ時のディープスリープ機能による省電力化も実装されています。

## 機能
*   **Wi-Fi接続:** 指定されたSSIDとパスワードでWi-Fiネットワークに接続します。
*   **時刻同期:** NTPサーバーから正確な時刻を取得し、表示します。
*   **乗り換え案内表示:** 
    *   定義された複数の路線（駅間）の乗り換え案内をNAVITIME APIから取得します。
    *   ボタン操作（短押し・長押し）で、表示する路線や時刻表を切り替えることができます。
    *   選択された路線の次の電車時刻表を表示します。
*   **遅延・運転見合わせ情報:** 
    *   路線情報取得時に、遅延や運転見合わせが発生している場合、その旨と詳細（URLから抽出）をLCDディスプレイに表示します。
*   **LCDディスプレイ表示:** 
    *   Arduino_GFXライブラリを使用し、ESP32 QSPIインターフェースに接続されたLCDディスプレイに情報を表示します。
...
## ファイル構成
*   `main.ino`: プログラムのメインファイル。Wi-Fi接続、NTP同期、LCD表示、ボタン処理、NAVITIME APIからのデータ取得と表示ロジックが含まれます。
...
3.  **ESP32へのプログラム書き込み:** 
    ...
    *   `pin_config.h` で定義されているディスプレイのマクロ (`DO0143FMST10` または `DO0143FAT01`) が、お使いのLCDに合わせて正しく選択されていることを確認してください。

4.  **動作確認:** 
    *   ESP32が起動し、Wi-Fiに接続されると、LCDディスプレイにトレインスケジュールビューアが表示され、時刻と路線情報が更新されます。
